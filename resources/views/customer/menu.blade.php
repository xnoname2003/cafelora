    ...
    <nav class="w-full bg-soft-brown-300 shadow-md sticky top-0 z-50">
        <div class="max-w-7xl mx-auto flex justify-between items-center py-4 px-8">
            ...
            <form action="{{ url()->current() }}" method="GET" class="flex justify-end w-full max-w-xs relative">
                ...
                <input type="text" placeholder="Cari menu..." name="search" value="{{ request('search') }}"
    ...
    <div class="pt-8">
        ...
        <header class="max-w-7xl mx-auto mb-12 px-8">
            <div class="relative w-full h-72 rounded-xl overflow-hidden shadow-xl">
                ...
                <img src="{{ asset('storage/bg/istockphoto-498240015-612x612.jpg') }}"
                ...
                <section>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                        ...
                        @foreach ($category->menus as $menu)
                            <div
                                class="card bg-soft-brown-100 rounded-2xl shadow-lg hover:shadow-xl transition duration-500 ease-in-out overflow-hidden transform cursor-pointer">

                                <div class="relative h-56 w-full">
                                    <img 
                                        src="{{ Str::startsWith($menu->image, 'http') 
                                            ? $menu->image : asset('storage/' . $menu->image) }}"
                                        alt="Gambar {{ $menu->name }}" alt="Gambar {{ $menu->name }}"
                                ...
                                <div class="p-6">
                                    ...
                                    <div class="flex justify-between items-center mb-4">
                                        ...
                                        <div class="text-right">
                                            <p
                                                class="text-sm font-medium {{ $menu->stock > 5 ? 'text-green-600' : 'text-red-500' }}">
                                                Stok: {{ $menu->stock }}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="space-y-3">
                                        ...
                                        {{-- VARIAN --}}
                                        <div>
                                            <span class="text-xs font-semibold text-soft-brown-700 block mb-1">Pilihan
                                                Varian:</span>
                                            ...
                                            <div class="flex flex-wrap gap-2 min-h-[28px]">
                                                @if ($menu->variants->isNotEmpty())
                                                    @foreach ($menu->variants as $variant)
                                                        <span
                                                            class="px-3 py-1 bg-soft-brown-100 text-soft-brown-500 border border-soft-brown-500 rounded-full text-xs font-medium shadow-sm">
                                                            {{ $variant->name }}
                                                        </span>
                                                    @endforeach
                                                @else
                                        ...
                                        {{-- TOPPING --}}
                                        <div>
                                            <span class="text-xs font-semibold text-soft-brown-700 block mb-1">Topping
                                                Tersedia:</span>
                                            ...
                                            <div class="flex flex-wrap gap-2 min-h-[28px]">
                                                @if ($menu->toppings->isNotEmpty())
                                                    @foreach ($menu->toppings as $topping)
                                                        <span
                                                            class="px-3 py-1 bg-soft-brown-500 text-soft-brown-100 rounded-full text-xs font-medium shadow-sm">
                                                            {{ $topping->name }}
                                                        </span>
                                                    @endforeach
                                                @else
