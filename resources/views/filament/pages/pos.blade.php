<x-filament-panels::page>
    <div class="flex flex-col lg:flex-row gap-6 w-full max-w-screen-xl mx-auto items-start">
        ...
        {{-- KERANJANG --}}
        <div class="hidden lg:block lg:w-[400px] flex-none bg-white dark:bg-gray-800 shadow-xl rounded-lg p-6 sticky top-6"> 
            <h2 class="text-2xl font-extrabold text-primary-600 mb-4 border-b dark:border-gray-700 pb-2">Keranjang Belanja</h2>
            
            <div class="overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-lg p-3 space-y-4" style="max-height: calc(100vh - 150px); width:275px;">
        ...
        <div class="flex-1 w-full min-w-0 bg-white dark:bg-gray-800 shadow-xl dark:shadow-2xl rounded-lg p-6">
            <h2 class="text-2xl font-extrabold text-primary-600 mb-4 border-b dark:border-gray-700 pb-2">Daftar Menu</h2>

            {{-- Filter kategori --}}
            <div class="flex flex-wrap gap-3 mb-4 bg-white dark:bg-gray-800 z-10 py-3 -m-3 border-b dark:border-gray-700 shadow-sm">
            ...
            <div class="overflow-y-auto" style="max-height: calc(100vh - 120px);"> 
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4 pb-4">
                    @foreach($this->menus as $menu)
                        <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-700 transition duration-300 hover:shadow-2xl flex flex-col h-full">
                            ...
                            @else
                                <div class="bg-gray-100 dark:bg-gray-700 rounded-t-xl h-40 flex items-center justify-center">
                            ...
                            <div class="p-4 flex flex-col flex-grow">
                                ...
                                <h3 class="font-bold text-xl text-gray-900 dark:text-white leading-snug">{{ $menu->name }}</h3>
                                <p class="text-xs text-primary-500 font-semibold mb-3">{{ $menu->category->name ?? 'Kategori' }}</p>
                                ...
                                <div class="text-base text-gray-700 dark:text-gray-200 mb-4 flex-grow">
                                    ...
                                    @else
                                        <span class="text-xs font-normal text-gray-500 block mt-1">Stok: {{ $menu->stock ?? '-' }}</span>
                                    @endif
                                        ...
                                        <div class="mt-2 text-xs text-gray-600 dark:text-gray-400 space-y-1">
                                            @if ($menu->variants->count())
                                                <div class="grid grid-cols-2 gap-x-2">
                                                    ...
                                                    <span class="font-normal">{{ $menu->variants->pluck('name')->join(', ') }}</span>
                                                </div>
                                            @endif
                                            @if ($menu->toppings->count())
                                                <div class="grid grid-cols-2 gap-x-2">
                                                    <span class="font-bold">Topping:</span>
                                                    <span class="font-normal">{{ $menu->toppings->pluck('name')->join(', ') }}</span>
                                                </div>
                                            @endif
                                        </div>
                                    @endif
    ...
    @if($showCart)
        <div class="fixed inset-0 bg-black/50 flex justify-center items-end lg:hidden z-50">
            <div class="bg-white dark:bg-gray-800 w-full rounded-t-xl p-4 max-h-[90vh] overflow-y-auto relative"> 
                ...
                <h2 class="text-xl font-bold mb-4 pt-2 text-center border-b dark:border-gray-700 pb-2">Keranjang Belanja</h2>
   ...
   {{-- Preview Struk --}}
    ...
    @if($transaction)
        ...
        <div class="flex justify-end gap-2 mt-4">
            ...
            <x-filament::button 
                wire:click="$dispatch('close-modal', { id: 'receipt-preview' })" 
                color="danger">
                Tutup
            </x-filament::button>
        </div>
