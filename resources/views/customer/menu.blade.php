            ...
            <div class="flex flex-col md:flex-row md:justify-between md:items-center w-full">
                ...
                <div class="flex justify-between items-center w-full md:w-auto">
                    ...
                    <div class="flex items-center space-x-2">
                        <a href="/"><span class="text-2xl md:text-3xl font-extrabold text-soft-brown-100">Cafelora</span></a>
                    </div>

                    {{-- Tombol Burger (Hanya tampil di mobile/layar kecil) --}}
                    <button id="filter-toggle"
                        class="md:hidden p-2 text-soft-brown-700 bg-soft-brown-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-soft-brown-700">
                        <svg id="icon-menu" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M4 6h16M4 12h16M4 18h16" />
                        </svg>
                        <svg id="icon-close" class="h-6 w-6 hidden" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <form action="{{ route('customer.menu.index') }}" method="GET" class="w-full md:w-auto"
                    {{-- Atur lebar otomatis di desktop --}} id="filter-menu">

                    <div id="filter-content"
                        class="md:flex md:space-x-3 hidden md:visible flex-col md:flex-row gap-3 mt-3 md:mt-0 items-end">

                        {{-- Input Cari --}}
                        <div class="relative w-full md:w-auto min-w-[180px]">
                            <input type="text" placeholder="Cari nama menu..." name="search"
                            ...
                            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                                <svg class="h-5 w-5 text-soft-brown-700" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        {{-- Input Harga Min --}}
                        <input type="number" placeholder="Harga Min" name="min_price"
                        ...
                        {{-- Input Harga Max --}}
                        <input type="number" placeholder="Harga Max" name="max_price"
                        ...
                        @if (request('search') || request('min_price') || request('max_price') || request('category'))
                            <a href="{{ route('customer.menu.index') }}"
                                class="w-full md:w-auto px-4 py-2 bg-red-600 text-soft-brown-100 rounded-lg font-semibold hover:bg-red-700 text-center transition duration-150 text-sm md:text-base">
                                Reset
                            </a>
                        @endif
                    </div>
    ...
    <div class="pt-6 md:pt-8">
        ...
        <header class="max-w-7xl mx-auto mb-8 md:mb-12 px-4 md:px-8">
            <div class="relative w-full h-48 md:h-72 rounded-xl overflow-hidden shadow-xl">
                ...
                <img src="{{ asset('images/banner-header/bg-header.jpg') }}"
        ...
        <div class="max-w-7xl mx-auto space-y-12 md:space-y-20 px-4 md:px-8">
            ...
            @if ($bestsellers->isNotEmpty() && !($isSearching ?? false))
                <section class="bg-soft-brown-100 p-6 md:p-8 rounded-xl shadow-2xl border-b-8 border-soft-brown-700">
                    <div
                        class="flex flex-col md:flex-row items-start md:items-center justify-between mb-6 md:mb-8 space-y-2 md:space-y-0">
                        <h2
                            class="text-2xl md:text-4xl font-extrabold text-soft-brown-700 uppercase tracking-wider flex items-center">
                            <svg class="w-6 h-6 md:w-8 md:h-8 mr-2 md:mr-3 text-red-600" fill="currentColor"
                        ...
                        <span
                            class="text-base md:text-xl font-semibold text-soft-brown-500 bg-soft-brown-200 px-3 py-1 md:px-4 md:py-2 rounded-full">
                            Terlaris Saat Ini!
                        </span>
                    </div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
                        @foreach ($bestsellers as $menu)
                            ...
                            <a href="{{ $detailUrl }}"
                                ...
                                <div
                                    class="absolute top-2 left-2 bg-red-600 text-white text-xs font-bold px-3 py-1 rounded-full shadow-lg z-20 transform rotate-1">
                                ...
                                <div class="p-3 md:p-4">
                                    <h3 class="text-lg md:text-xl font-bold mb-1 text-soft-brown-700 fixed-title">
                                        {{ $menu->name }}</h3>

                                    <hr class="my-2 md:my-3 border-soft-brown-300">
                                    <div class="flex justify-between items-center">
                                        <p class="text-lg md:text-xl font-extrabold text-soft-brown-500">Rp
                                            {{ number_format($menu->base_price, 0, ',', '.') }}</p>
                                        <div class="text-right">
                                            <span
                                                class="text-soft-brown-500 text-xs uppercase font-medium block">Terjual</span>
                                            <p class="text-sm md:text-md font-bold {{ $salesColor }}">
                                                {{ $salesQty }}x</p>
                                        </div>
                                    </div>
                                </div>
            ...
            @if (isset($isSearching) && $isSearching)
                ...
                <section>
                    ...
                    @if ($filteredMenus->isEmpty())
                        <p class="text-lg text-soft-brown-500 italic">Menu tidak ditemukan dengan filter yang
                            diterapkan.</p>
                    @else
                        @foreach ($groupedMenus as $categoryName => $menus)
                            <h3 class="text-xl md:text-2xl font-bold text-soft-brown-700 mb-4 mt-8">
                                {{ $categoryName }}</h3>

                            <div
                                class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8 mb-8 md:mb-10">
                                @foreach ($menus as $menu)
                                    ...
                                    <a href="{{ $detailUrl }}"
                                        ...
                                        <div class="p-4 md:p-6">
                                            <h3
                                                class="text-xl md:text-2xl font-bold mb-1 text-soft-brown-700 fixed-title">
                                                {{ $menu->name }}</h3>

                                            <p class="mt-2 text-soft-brown-500 text-xs md:text-sm fixed-description">
                                                {{ $shortDescription ?? 'Deskripsi tidak tersedia.' }}</p>

                                            <hr class="my-3 md:my-4 border-soft-brown-200">
                                            <div class="flex justify-between items-center mb-3 md:mb-4">
                                                <div class="flex flex-col">
                                                    <span
                                                        class="text-soft-brown-500 text-xs uppercase font-medium">Harga</span>
                                                    <p class="text-xl md:text-2xl font-extrabold text-soft-brown-500">
                                                        Rp {{ number_format($menu->base_price, 0, ',', '.') }}</p>
                                                </div>
                                                <div class="text-right">
                                                    <span
                                                        class="text-soft-brown-500 text-xs uppercase font-medium block">Terjual</span>
                                                    <p class="text-base md:text-lg font-bold {{ $salesColor }}">
                                                        {{ $salesQty }}x</p>
                                                </div>
                                            </div>
                                            ...
                                            <div class="space-y-3 fixed-option-area">
                                                    ...
                                                    <div class="flex flex-wrap gap-2 min-h-7">
                                                        @if ($menu->variants->isNotEmpty())
                                                            @foreach ($menu->variants as $variant)
                                                                <span
                                                                    class="px-2 py-0.5 md:px-3 md:py-1 bg-soft-brown-100 text-soft-brown-500 border border-soft-brown-500 rounded-full text-xs font-medium shadow-sm">
                                                                    {{ $variant->name }}
                                                                </span>
                                                            @endforeach
                                                        @else
                                                            <span class="text-soft-brown-400 text-xs italic">Tidak ada
                                                                varian</span>
                                                        @endif
                                                    </div>
                                                </div>
                                                ...
                                                <div>
                                                    <span
                                                        class="text-xs font-semibold text-soft-brown-700 block mb-1">Topping
                                                        Tersedia:</span>
                                                    <div class="flex flex-wrap gap-2 min-h-[28px]">
                                                        @if ($menu->toppings->isNotEmpty())
                                                            @foreach ($menu->toppings as $topping)
                                                                <span
                                                                    class="px-2 py-0.5 md:px-3 md:py-1 bg-soft-brown-500 text-soft-brown-100 rounded-full text-xs font-medium shadow-sm">
                                                                    {{ $topping->name }}
                                                                </span>
                                                            @endforeach
                                                        @else
                        ...
                        <section>
                            <a href="{{ route('customer.category.show', ['name' => $category->name]) }}">
                                <h2
                                    ...
                                    {{ $category->name }}
                                    <svg class="inline-block align-middle" width="35" height="35"
